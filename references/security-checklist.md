...
本清單是 `interactive-skill-architect` 的**單一資安真相**，由**資安稽核模式（模式 C）獨家承接**：

- **資安稽核模式（Phase S2 診斷）**：套用**全部 4 個維度（SEC-1~SEC-4）**，逐條偵測並套用風險分級，輸出獨立資安報告。

> **為何不由優化模式加掛**：資安是專門切面，需要獨立的報告格式（Critical/High/Medium/Low）與修補閘門。過去曾讓優化模式「全面健檢＋資安深掃」加掛本清單，導致兩條半套資安路徑並存、易混淆。現已收斂為單一入口：優化模式只在第 12 項做 `scripts/` 表層安全紅線；任何深度四維度稽核一律走模式 C（優化診斷若嗅到資安疑點，會主動詢問是否轉模式 C，見 `optimize-mode.md` Phase O2）。
...
優化模式的**第 12 項「腳本品質與安全分析」**只做 `scripts/` 的**表層安全紅線**（硬編碼密鑰、`rm -rf`、`curl | sh`），屬品質健檢的一環。本清單是**深度資安稽核的專屬真相**，涵蓋第 12 項不做的：Prompt Injection（SEC-3）、資料外洩與外部呼叫（SEC-2/SEC-4）、設定檔/manifest 自動執行面、跨平台（Windows/多語言），並改用風險分級。**兩者不加掛、不並存**：第 12 項發現疑點時，由優化模式主動引導使用者轉入模式 C（本清單），而非在優化流程內臨時套用本清單。命中的紅線項不得降級。
