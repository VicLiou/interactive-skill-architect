...
- **供應鏈風險**：安裝未鎖版或來源不明的相依——`pip install` 任意來源、`npm i`、`gem install`、`go install`、Windows `Install-Module`/`choco`/`winget`/`nuget`、`curl latest | …`、下載第三方程式未做 hash/簽章校驗。→ 🟠 High（來源可信但未鎖版 → 🟢 Low）
...
- **二進位無法掃描**：掃描器會列出略過的二進位檔（尤其 `scripts/` 下的執行檔／`.pyc`／pickle），這些**無法**做內容掃描，須人工確認用途與來源。
- **符號連結**：掃描器會提示 symlink，須確認是否指向資料夾外（可能被用來讀寫敏感位置）。
