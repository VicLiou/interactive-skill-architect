...
案例對照 skill 的硬性閘門與 gotcha，安全類（破了會出事的）優先覆蓋（目前 14 案例）：
- **建立**：預填捷徑、禁止合併提問、先確認才寫檔＋0 FAIL、空殼 reference、Gotchas 留空主動推薦、Q4 動態追問。
- **藍本**：藍本唯讀、退化成複製要停止（安全類）。
- **優化**：fail-closed 缺 SKILL.md、過度修正／先確認才覆寫（安全類）、時效性幻覺。
- **資安**：不只掃三慣例資料夾、合法宣告用途不誤判（誤報／漏報兩向）、整段編碼混淆漏報、修補後回歸一致性＋防掉包。
